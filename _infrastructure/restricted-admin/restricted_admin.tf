resource "aws_iam_role" "restricted_admin" {
  name               = "qwixxTerraformRestrictedAdmin"
...
resource "aws_iam_policy" "restricted_admin" {
  name        = "qwixx-restriced-admin-policy"
...
data "aws_iam_policy_document" "restricted_admin" {
  ...
  statement {
    ...
    resources = ["arn:aws:cloudfront::643625685022:distribution/E96GBKAUTW52Y"
    ]
  }
}
